import { _decorator, Component, Node, SkeletalAnimationComponent, Animation, BoxCollider, ParticleSystem, ICollisionEvent, RigidBody, Vec3 } from 'cc';
...
@ccclass('BarrelChop')
export class BarrelChop extends Component {
    ...
    _onTriggerEnter(event: ICollisionEvent) {
        ...
        if (Constant.ismoving == false) {
            ...
            if (otherNode === this.podium) {
                if (this.weaponMgt.getCurrentWeaponState() == 2) {
                    this.ifcollisionOver = false;
                    this.startChopping();
                } else {
    ...
    _onTriggerExit(event: ICollisionEvent) {
        ...
        //  console.log("OnTriggerExit",this.node)
    ...
    startChopping() {
        ...
        if (animState) {
            ...
            AudioManager.playSound("barrealAudio");


            this.scheduleOnce(() => {

                this.explosionOne.getComponent(ParticleSystem).play();
                this.explosionTwo.getComponent(ParticleSystem).play();
                this.explosionThree.getComponent(ParticleSystem).play();
                clientEvent.dispatchEvent(Constant.EVENT_TYPE.ScoreCounter);
                this.node.getComponent(Animation).play("barrelcompress");
            }, 0.5);
